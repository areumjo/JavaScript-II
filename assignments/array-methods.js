...
ticketPriceTotal.push(runners.reduce((accu, current) => accu += current.donation, 0));
...
// Problem 1
// Divide the ruuners by the shirt size and see how many shirts with each size are needed. Check the sum is 50 as well.

let xsSizeNumber = runners.filter( ele => ele.shirt_size === 'XS').length;
let sSizeNumber = runners.filter( ele => ele.shirt_size === 'S').length;
let mSizeNumber = runners.filter( ele => ele.shirt_size === 'M').length;
let lSizeNumber = runners.filter( ele => ele.shirt_size === 'L').length;
let xlSizeNumber = runners.filter( ele => ele.shirt_size === 'XL').length;
let xxlSizeNumber = runners.filter( ele => ele.shirt_size === '2XL').length;
let xxxlSizeNumber = runners.filter( ele => ele.shirt_size === '3XL').length;
 
console.log(`X-small size: ${xsSizeNumber}`)
console.log(`Small size: ${sSizeNumber}`)
console.log(`Medium size: ${mSizeNumber}`)
console.log(`Large size: ${lSizeNumber}`)
console.log(`XL-large size: ${xlSizeNumber}`)
console.log(`2XL-large size: ${xxlSizeNumber}`)
console.log(`3XL-small size: ${xxlSizeNumber}`)
console.log(`Total number: ${xsSizeNumber + sSizeNumber + mSizeNumber + lSizeNumber + xlSizeNumber + xxlSizeNumber + xxxlSizeNumber}`);



// Problem 2
// After the event director saw the result of problem 1, he/she changed mind. The director wants to see who has what size.
// Make an object with information of shirt size (key) and first name (value)

// @@(1) use function with a string (shirt_size) parameter that shows a list of runners with the shirt size
const sizeTell = function(size){
	let sizeTeller = [];
	runners.filter( ele => ele.shirt_size === size).forEach((ele) => sizeTeller.push(`${ele.first_name} ${ele.last_name} is ${size} size`)); 
	return console.log(sizeTeller);
}
// @@(2) call the function with the size you want to know
sizeTell('XS');
sizeTell('XL');
// @@(3) now the director is happy! just type sizeTell('size') then it shows the runners with first name and last name



// Problem 3
// The event director wants to make a platinum and gold badge for the runners who donates more than 200 and 100 respectively. 
// Return the two objects named 'goldMember' and 'platinumMember' having the badge level (key) and the member's full name (value)

const gold = runners.filter( money => money.donation >= 100 && money.donation < 200).map(ele => `${ele.first_name} ${ele.last_name}`);
const goldMember = {};
goldMember["Gold-badge"] = gold;

const platinum = runners.filter( money => money.donation > 200).map(ele => `${ele.first_name} ${ele.last_name}`);
const platinumMember = {};
platinumMember["Platinum-badge"] = platinum;

console.log(goldMember);
console.log(platinumMember);